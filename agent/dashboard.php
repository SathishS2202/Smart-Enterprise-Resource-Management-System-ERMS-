...
$task_counts = mysqli_fetch_assoc(mysqli_query($conn,"SELECT SUM(status='To Do') AS todo, SUM(status='In Progress') AS in_progress, SUM(status='Done') AS done FROM tasks WHERE assigned_to=$agent_id"));
$attendance_counts = mysqli_fetch_assoc(mysqli_query($conn,"SELECT SUM(CASE WHEN check_in IS NOT NULL THEN 1 ELSE 0 END) AS present, SUM(CASE WHEN check_in IS NULL THEN 1 ELSE 0 END) AS absent FROM attendance WHERE user_id=$agent_id"));

/* Recent tasks */
$recent_tasks = mysqli_query($conn,"SELECT t.*, p.project_name FROM tasks t JOIN projects p ON t.project_id=p.id WHERE t.assigned_to=$agent_id ORDER BY t.id DESC LIMIT 5");
...
.card h5{margin-bottom:10px;color:#374151}
...
<!-- Third Row: Recent Tasks & Projects -->
<div class="row g-3">
    <div class="col-md-6">
        <div class="card table-container">
            ...
            <tr><td><?= htmlspecialchars($t['title']) ?></td><td><?= htmlspecialchars($t['project_name']) ?></td><td><?= $t['status'] ?></td></tr>
            <?php } ?>
            </tbody>
            </table>
        </div>
    ...
    <div class="col-md-6">
        <div class="card table-container">
            ...
            <tr><td><?= htmlspecialchars($p['project_name']) ?></td><td><?= $p['status'] ?></td><td><?= $p['start_date'] ?></td></tr>
            <?php } ?>
            </tbody>
            </table>
        </div>
...
<script>
const taskPie = new Chart(document.getElementById('taskPie'),{
    type:'pie',
    data:{
        labels:['To Do','In Progress','Done'],
        datasets:[{data:[<?= $task_counts['todo'] ?? 0 ?>,<?= $task_counts['in_progress'] ?? 0 ?>,<?= $task_counts['done'] ?? 0 ?>],
        backgroundColor:['#2563eb','#f59e0b','#16a34a']}]},
    options:{plugins:{title:{display:true,text:'Task Status'}}}
});

const projectBar = new Chart(document.getElementById('projectBar'),{
    type:'bar',
    data:{
        labels:['Projects'],
...
const attendanceLine = new Chart(document.getElementById('attendanceLine'),{
    type:'line',
    data:{
